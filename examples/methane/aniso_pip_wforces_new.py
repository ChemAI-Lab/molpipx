...
from pipjax import get_functions, get_f_mask, get_pip_grad
...
def lambda_random_init(params_pip: Any, key: Any):
    ...
    w_rnd = jrnd.uniform(key, shape=(w_l.shape), minval=0.9, maxval=2.25)
...
def main():
    ...
    (X_tr, F_tr, y_tr), (X_val, F_val, y_val) = split_train_and_test_data_w_forces(
        X_all, F_all, y_all, 100, key, 100)
    atoms = atoms_all[0]
    data = (X_tr, F_tr, y_tr), (X_val, F_val, y_val)

    molecule_type = 'A4B'
    poly_degree = 3
    opt_info = {'tol': 1e-4, 'Maxiters': 10000, 'learning_rate': 1E-1}
