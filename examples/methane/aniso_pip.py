...
def _train_aniso_bfgs(molecule_type, poly_degree, data, atoms,  optimizer_info):
    ...
    model_pip = LayerPIPAniso(f_mono, f_poly, n_pairs)
    params_pip = model_pip.init(key, X_tr[:1], mask)
    params_pip = lambada_random_init(
        params_pip, key)  # random initial parameters
    ...
    @jax.jit
    def validation_loss(params_pip, params_energy):
        (X_tr, y_tr), (X_val, y_val) = data
    ...
    l_trj = []
    for i in range(n_epochs):
        solver = jaxopt.LBFGS(fun=validation_loss, maxiter=1, has_aux=True)
        res = solver.run(params_pip, params_energy=params_energy)
        params_pip = res.params
        li = params_pip['params']['VmapJitPIPAniso_0']['lambda']
        l_trj.append(li)
        print(res.state)
        print(params_pip)
        print(li)
    return l_trj
    # print('init', params_pip)
    # print('opt', res.params)
...
def plotting(l_opt_trj, molecule_type, poly_degree, atoms, data):
    ...
    plt.text(0.875, 1.85, 'b)', fontsize=18)
    plt.xlim(0.8, 2.0)
    plt.ylim(0.8, 2.0)
    ...
    fig_file = 'loss_val_grid_Adam_n250.png'
