...
def generate_msa_file_generator(n_A: int = 2, poly_degree: int = 3):

    # filename = f'MOL_{n_A}_{poly_degree}'
    path = '/home/ravh011/molpipx/molpipx/msa_files/molecule_A2'
    label = f'_MOL_{n_A}_{poly_degree}'
    for p in range(3, 9):  # max degree generated was 6
...
def test_mono_files(poly_degree: int = 3):
    ...
    x_mono = f_mono(distance)
    x_poly = f_poly(distance)

    print(x_mono.shape, x_poly.shape)


if __name__ == "__main__":
    generate_msa_file_generator()
    # test_mono_files(poly_degree=6)
    # assert 0
    # test_mono_files(poly_degree=3)
    # test_mono_files(poly_degree=4)
    # test_mono_files(poly_degree=5)
