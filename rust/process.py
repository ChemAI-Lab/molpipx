...
folders = os.listdir("../molpipx/msa_files")
...
for folder in folders:
    ...
    cmd = f"unzip -j -qq ../molpipx/msa_files/{folder}/*_files.zip -d data/{folder} -x \"*.BAS\" -x \"*.FOC\" -x \"*.MAP\""
    print("running: ", cmd)
    subprocess.run(["unzip", "-j", "-qq", f"../molpipx/msa_files/{folder}/*_files.zip", "-d", f"data/{folder}", "-x", "*.BAS", "-x", "*.FOC", "-x", "*.MAP"], stderr=subprocess.DEVNULL)
